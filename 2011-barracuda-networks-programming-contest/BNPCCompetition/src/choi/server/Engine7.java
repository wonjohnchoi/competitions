...
public class Engine7 {
    ...
    int totalLines[] = new int[6];
    int totalGames[] = new int[6];

    public boolean isGoodForThem(int val, int interruptAt) {
        int pre_score = eval(theirs);
        theirs.add(val);
        int cur_score = eval(theirs);
        theirs.remove(new Integer(val));
        return pre_score < cur_score && cur_score >= interruptAt;
    }
    ...
    public int getIdx(int val, boolean forced, int interruptAt) {
        ...
        if (! forced && fixed[idx] && (preVal >= newVal) && !isGoodForThem(val, interruptAt)) {
            return -1;
        }
        rack[idx] = val;
        fixed[idx] = true;
