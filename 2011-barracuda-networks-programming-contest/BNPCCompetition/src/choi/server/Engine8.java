...
public class Engine8 {
    ...
    public boolean isGoodForThem(int val) {
        ...
        return pre_score < cur_score && cur_score >= 4 && pre_score < 5;
    }
    ...
    public int eval(ArrayList<Integer> cards) {
        ...
        for(int i = 1; i < cards.size(); i += 1) {
            if(cards.get(i) == cards.get(i - 1) + 1) {
                size += 1;
            } else if (size == 4 && garbage.contains(cards.get(i-1)+1) && (i==4 || garbage.contains(cards.get(i-4) - 1))) {
                size = 1;
                continue;
            } else {
        ...
        if (size == 4 && garbage.contains(cards.get(cards.size()-1) + 1)) {
            return best;
        } else {
