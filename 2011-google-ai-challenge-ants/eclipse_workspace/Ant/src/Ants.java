...
public class Ants {
    ...
    public void issueOrder(Tile myAnt, Aim direction) {
        Tile target = new Tile((myAnt.getRow() + direction.getRowDelta() + rows) % rows, (myAnt.getCol() +direction.getColDelta() +cols)%cols);
        Order order = new Order(myAnt, target, direction.getSymbol());
        orders.add(order);
