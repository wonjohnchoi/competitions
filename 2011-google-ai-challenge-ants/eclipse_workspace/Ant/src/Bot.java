...
public abstract class Bot extends AbstractSystemInputParser {
    ...
    public void afterUpdate() {
        visible = new boolean[ants.getRows()][ants.getCols()];
        for (Tile ant: ants.getMyAnts()) {
            for (int r = 0; r < ants.getRows(); r += 1) {
                for (int c = 0; c <ants.getCols(); c += 1) {
                    if (ants.getDistance(new Tile(r, c), ant) <= ants.getViewRadius2()) {
                        visible[r][c] = true;
                    }
                }
            }
