...
public class Handler7 {
    ...
    public HashMap<String, Object> get_move(HashMap<String, Object> data) {  
        ...
        engine.nMoves += 1;
        int idx = engine.getIdx(discard, false, interruptAt);        
    ...
    public int get_deck_exchange(HashMap<String, Object> data) {
        ...
        int idx = engine.getIdx(card, true, interruptAt);
    ...
    public String game_result(HashMap<String, Object> data) {
        ...
        engine.totalLines[interruptAt] += engine.nMoves;
        engine.totalGames[interruptAt] += 1;
        for(int i = 3; i <= 5; i += 1) {
            if (engine.totalGames[i]==0)continue;
            reportln("Avg. line# for interrupting before " + i + ": " + engine.totalLines[i]/engine.totalGames[i]);
        }
        interruptAt = Math.max((interruptAt+1)%6, 3);
        engine.reset();
