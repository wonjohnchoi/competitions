...
def solve():
    fout = open('C:\\Users\\Wonjohn Choi\\Python\\Google Code Jam\\B.out','w')
    fin = open('C:\\Users\\Wonjohn Choi\\Python\\Google Code Jam\\B.in','r')
    
    t = int(next(fin))
    for i in range(t):
        line = map(str, str(next(fin)).strip().split(' '))
        #print(line)
        ...
        #print('['+', '.join(convert(inputs, combines, opposes))+']')
        fout.write('Case #'+str(i+1)+': ['+', '.join(convert(inputs, combines, opposes))+']')
        fout.write('\n')
    fout.close() 
    fin.close()
