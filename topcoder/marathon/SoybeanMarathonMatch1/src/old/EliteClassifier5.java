...
public class EliteClassifier5 {
    ...
    static final String DESCRIPTION = "For each experiment, penalize singled-tested data. optimize with penalizing experiments.";
    ...
    //ExperimentSummary.csv
    //ExperimentChartSectionToElite.csv
    public static void analyzeExperiments() {
        System.out.println("accessing analyzeExperiments...");
        Scanner all = open(BASE + "DataTraining.scv");

        PrintWriter out = null;
        try {
            out= new PrintWriter(new FileWriter("ExperimentChartSectionToElite.csv"));
        } catch (IOException e) {
        ...
        while (all.hasNextLine()) {
            entries.add(new Entry(all.nextLine()));
        }

        ListIterator<Entry> iter = entries.listIterator();

        int[][] chart = new int[50][50];
        //experimentId,sectionSize,#Check,#Elite
        while (iter.hasNext()) {
            Experiment experiment = new Experiment(iter);
            
            int base = experiment.sections.get(0).entries.size();
            for (int i = 0; i < experiment.sections.size(); i += 1) {
                if (base != experiment.sections.get(i).entries.size()) {
                    System.out.println("Broken Experiment: " + experiment.ExperimentId);
                    for (Section section : experiment.sections) {
                        System.out.println(section.entries.size());
                    }
                    continue;
                }
            }
            ...
            int check = 0;
            int elite = 0;
            for (Entry entry : experiment.sections.get(0).entries) {
                if (entry.IsCheck) {
                    check += 1;
            ...
            chart[experiment.sections.size()][elite] += 1;
            
            
            /*
            out.println(experiment.ExperimentId+","+experiment.sections.size()
                +","+check+","+elite);*/
            
        }
        
        /*
        for (int r = 0; r < chart.length; r += 1) {
            for (int c = 0; c < chart.length; c += 1) {
                if (c != 0) {
                    out.print(",");
                }
                out.print(chart[r][c]);
    ...
    public static int[] classify3(String[] data, String[] locations) {
        System.out.println("Accessing classify...");
        ...
        Collections.sort(entries, new Comparator<Entry>() {
            ...
            public int compare(Entry arg0, Entry arg1) {
                return ((Double)arg1.Yield).compareTo(arg0.Yield);
            }
        ...
        System.out.println("Exiting classify...");
        return result;   
    }
    
    public static void evalClassify() {
        ...
        Double value() {
            if (value == null) {
                if (entries.get(0).IsCheck) {
                    ...
                    value = totalScore / nScore * entries.get(0).secton.experiment.value();
                    //System.out.println(value);
                    //System.out.println(deviation(yields));
                    deviations[(int)deviation(yields)] += 1;

                    if (deviation(yields) > 100) {
                        System.out.println("YIELDS");

                        for (Entry entry : entries) {
                            System.out.println(entry.Yield);
                        }
                    }
                }
            }
