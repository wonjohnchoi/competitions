...
public class EliteClassifier9 {
    ...
    static class Analyzer {
        ...
        public int[] classify(Constants constants) {
            ...
            //TEST
            ArrayList<Variety> lowMN = new ArrayList<Variety>();
            for (Variety variety : varieties) {
                Double mn = variety.avgMN();
                if (mn != null && mn < 110) {
                    lowMN.add(variety);
                    /*
                    if (variety.entries.get(0).VarietyID == 38326) {
                        System.out.println("FOUND");
                        System.out.println(variety.avgMN());
                        System.out.println(variety.avgYield());
                        System.exit(1);
                    }*/
                }
            }
            int nElite = 0;
            for (Variety variety : lowMN) {
                if (variety.entries.get(0).IsElite) {
                    nElite += 1;
                }
            }
            Collections.sort(lowMN, new Comparator<Variety>() {

                @Override
                public int compare(Variety arg0, Variety arg1) {
                    if (arg0.entries.get(0).IsCheck) {
                        return 1;
                    }
                    if (arg1.entries.get(0).IsCheck) {
                        return -1;
                    }
                    /*
                    double ratio0 = 1;
                    for (Entry entry : arg0.entries) {
                        ratio0 *= entry.Yield / entry.section.avgYield();
                    }
                    double ratio1 = 1;
                    for (Entry entry : arg1.entries) {
                        ratio1 *= entry.Yield / entry.section.avgYield();
                    }
                    return Double.compare(ratio1, ratio0);*/
                    return new Double(arg1.avgYield() / arg1.entries.get(0).section.avgYield()).compareTo(new Double(arg0.avgYield() / arg0.entries.get(0).section.avgYield()));
                }
            ...
            for (Variety variety : lowMN) {
                ...
                if (variety.entries.get(0).IsElite) {
                    correct += 1;
                    score += 1000000 /nElite * correct / total;
